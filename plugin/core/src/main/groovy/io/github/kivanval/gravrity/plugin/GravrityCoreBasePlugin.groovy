...
@CompileStatic
class GravrityCoreBasePlugin implements Plugin<Project> {
  private final Project project

  @Inject
  GravrityCoreBasePlugin(Project project) {
  ...
  private Configuration createAvroConfiguration(SourceSet sourceSet) {
    final def avroConfigName = getConfigName(sourceSet.name, "avro")
  ...
  private SourceDirectorySet createAvroSourceDirectorySet(SourceSet sourceSet) {
    ...
    final def avro = project.objects.newInstance(
      DefaultAvroSourceDirectorySet, project.objects.sourceDirectorySet("avro", displayName))
    avro.include("**./*.avdl", "**./*.avpr", "**/*.avsc")
    avro.srcDir("src/${sourceSet.name}/${avro.name}")

    sourceSet.extensions.add(DefaultAvroSourceDirectorySet, avro.name, avro)
